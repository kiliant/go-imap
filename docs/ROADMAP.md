...
The wire codec and the core vocabulary. No I/O, no client.

This milestone is deliberately serial and blocks everything else: it fixes the
types that appear in every later signature, and those are exactly the types that
...
Connection, TLS/STARTTLS, SASL, mailbox commands, message commands.
...
## M4 — Hardening and the freeze (T12, T13, T14, T15)

Interop harness completion, fuzzing corpus, API surface review, documentation and
examples, release engineering.
